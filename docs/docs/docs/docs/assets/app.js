...
/* harmony import */ var _videos__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./videos */ "./assets/js/videos.js");
...
function ScrollEnter(selector, callback, options) {
  ...
  this.elementsLoad = function () {
    ...
    for (var i = 0, nbEls = this.elements.length; i < nbEls; i++) {
      el = this.elements[i];
      ...
      if (approche) {
        console.log(el);
        callback(el); // this.elements.splice(i, 1)

        elSup.push(el); // delete this.elements[i]

        console.log(this.elements);
      }
    }

    for (var _i = 0, nb = elSup.length; _i < nb; _i++) {
      this.deleteElement(elSup[_i]);
    }
  };
...
document.addEventListener('DOMContentLoaded', function () {
  (0,_header__WEBPACK_IMPORTED_MODULE_0__.menuMobile)();
  var skillsPictures = document.querySelectorAll('.skills picture');

  if (skillsPictures) {
    var picture = null;

    for (var i = 0, skillsPicturesLength = skillsPictures.length; i < skillsPicturesLength; i++) {
  ...
  new ScrollEnter('picture', function (el) {
    ...
    var ext = el.dataset.ext;

    if (!src || !ext) {
      console.log('src non definie');
      return;
    }
    ...
    if (sources.length) {
      for (var _i2 = 0, nbEl = sources.length; _i2 < nbEl; _i2++) {
        source = sources[_i2];
  ...
  var videos = document.querySelectorAll('img.video');
  ...
  if (videos.length > 0) {
    new ScrollEnter('img.video', function (el) {
      ...
      el.parentNode.classList.add('active');
      var src = el.dataset.src;
      el.parentNode.appendChild((0,_videos__WEBPACK_IMPORTED_MODULE_1__.createVideo)(src, el.classList));
...
/* harmony export */ });
var classLinkMobile = '.js-link-mobile';
function menuMobile() {
  var link = document.querySelector(classLinkMobile);
  var ul = link.parentNode.querySelector('ul');
 ...
 */
function createVideo(src, classes) {
  var optionsVideos = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
